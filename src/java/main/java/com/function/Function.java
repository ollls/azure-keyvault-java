...
public class Function {
  ...
  public void run( @EventGridTrigger(name = "event") EventSchema event, final ExecutionContext context) {
    ...
    String appRegistrationObjectId_Full = event.subject;
    String[] parts = appRegistrationObjectId_Full.split( "_" );
    String appRegistrationObjectId = ( parts.length == 2 ) ? parts[0] : appRegistrationObjectId_Full;
    ...
    String previousClientSecretKeyId = secretClient
        .getSecret(appRegistrationObjectId_Full)
        .getProperties()
        .getContentType();
    ...
    if (newPasswordCredential != null) {
      KeyVaultSecret newKeyVaultSecret = new KeyVaultSecret(appRegistrationObjectId_Full, newPasswordCredential.secretText);
      ...
      try {
        context.getLogger().info("Updating secret in Key Vault for " + appRegistrationObjectId_Full);
  ...
  private PasswordCredential SetAppRegistrationClientSecret(
